...
- `bun run build`
  Builds the regular external binary at `./acode`.
- `bun run compile`
  Builds the regular external binary at `./dist/acode`.
- `bun run build:dev`
  Builds `./acode-dev` with a dev-stamped version and experimental GrowthBook key.
- `bun run build:dev:full`
  Builds `./acode-dev` with the entire current "Working Experimental Features"
